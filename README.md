...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create ping-pong-tournament-9f51
  fly apps create ping-pong-tournament-9f51-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app ping-pong-tournament-9f51
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app ping-pong-tournament-9f51-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'ping-pong-tournament-9f51-staging' does not match app name in config file 'ping-pong-tournament-9f51'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name ping-pong-tournament-9f51-db
  fly postgres attach --app ping-pong-tournament-9f51 ping-pong-tournament-9f51-db

  fly postgres create --name ping-pong-tournament-9f51-staging-db
  fly postgres attach --app ping-pong-tournament-9f51-staging ping-pong-tournament-9f51-staging-db
  ```
